...
async function run () {
	...
	await report("Checking for PRODUCT")
	...
	if (config.auto_submit == 'true') {
		...
		try {
			console.log('trying autosubmit is true');
			//await page.waitForSelector('#labeled-input-password' , {timeout: 2500})
			const [button] = await page.$x("//button[contains(., 'Continue to payment')]", {timeout: 2500})
			if (button) {
				await report("Continueing to payment")
				await button.click()
			}
		} catch (err) {
		}
		try {
			...
			//await page.focus("[placeholder='CVV2']", { timeout: 7000 })
			//await page.type("[placeholder='CVV2']", config.cv2)
			await page.waitForSelector("[placeholder='CVV2']", { timeout: 5000 })
			//await page.focus("[placeholder='CVV2']", { timeout: 7000 })
			//await page.type("[placeholder='CVV2']", config.cv2)
		...
		/*await page.evaluate(() => {
			...
			let elements = $('checkout-step-action-done').toArray();
			for (i = 0; i < elements.length; i++) {
			  $(elements[1]).click();
			  console.log('ME MONKEY OOGA BOOGA WE WAZ KANGZ'+elements[1]);
			}
		 });*/
	}
	...
	await page.focus("[placeholder='CVV2']", { timeout: 7000 })
		...
		const [button2] = await page.$x("//button[contains(., 'Review your order')]", { timeout: 7000 })
		if (button2) {
			await report("Reviewing Order")
			await button2.click()
		}
    	//await browser.close()
}
